...
export default function CoralineWorld({ mode, onEnter }: { mode: 'full' | 'return'; onEnter: () => void }) {
  ...
  // ── "Welcome back" beat — every day after the first, Taylor gets one warm,
  // single-screen greeting instead of the full journey. His time doing
  // homework matters more than watching the same five scenes on repeat; the
  // streak does the motivating here, quickly, then gets out of his way.
  if (mode === 'return') {
    return (
      <div ref={rootRef} className="fixed inset-0 z-50 flex flex-col items-center justify-center overflow-hidden px-6 text-center text-white"
        style={{ background: 'radial-gradient(1100px 750px at 50% -10%, #2a1560 0%, transparent 60%), linear-gradient(180deg,#0b1030,#160a38 55%,#0a0718)' }}
      >
        <div className="w-stars absolute inset-0" />
        <div className="mist w-drift-a" style={{ width: 420, height: 420, left: '-6%', top: '20%', background: '#7c3aed', opacity: .35 }} />
        <div className="mist w-drift-b" style={{ width: 380, height: 380, right: '-6%', bottom: '10%', background: '#db2777', opacity: .3 }} />
        <div className="absolute right-[14%] top-[16%] float-slow">
          <div className="w-glow absolute -inset-8 rounded-full blur-3xl" style={{ background: 'radial-gradient(circle, rgba(253,224,138,.5), transparent 65%)' }} />
          <svg width="60" height="60" viewBox="0 0 92 92" fill="none" className="relative">
        ...
        <button
          ...
          className={`glass-night fixed left-4 top-4 z-[60] flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold ring-1 ring-white/20 ${soundOn ? 'text-amber-200' : 'text-violet-100 hover:text-white'}`}
        >
          <span className="text-sm">{soundOn ? '🔊' : '🔈'}</span>
        </button>

        <div className="relative z-10 max-w-md reveal in">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.3em] text-fuchsia-200/70">다른 세계</p>
          <h2 className="font-display text-3xl font-extrabold leading-tight sm:text-4xl">
            다시 왔구나, <span className="text-shimmer">태윤아!</span>
          </h2>

          <div className="glass-night mx-auto mt-6 inline-flex flex-col items-center gap-2.5 rounded-2xl px-7 py-4">
            <div className="text-sm font-extrabold text-amber-200">
              {streak.count >= 2 ? `🔥 ${streak.count}일 연속 공부 중!` : '🔥 오늘도 불꽃을 이어가자!'}
            </div>
          ...
          <p className="mt-6 text-sm leading-relaxed text-violet-100/80">문이 태윤이를 기다리고 있었어. 오늘도 한 걸음 들어가 볼까?</p>

          <button
            onClick={enterWithFx}
            className="group mt-7 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-violet-500 via-fuchsia-500 to-indigo-600 px-7 py-3.5 text-base font-extrabold text-white shadow-2xl shadow-fuchsia-500/40 ring-1 ring-white/25 hover:-translate-y-0.5 hover:shadow-fuchsia-500/60"
          >
            <span>✨ 공부하러 들어가기</span>
