...
export default function ShadowingPlayer({ text }: Props) {
  ...
  const handlePlayPause = () => {
    if (!isActive) {
      play(sentences, Math.max(currentIndex, 0), rate, shadowMode, shadowPause);
    } else if (isPlaying) {
      pause();
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <div className="flex flex-wrap items-center gap-4 mb-4">
          ...
          >
            {isPlaying ? (
              <><span className="text-xl">⏸</span> 일시정지</>
            ) : state === 'paused' ? (
              <><span className="text-xl">▶️</span> 계속</>
            ) : (
          ...
          <button
            onClick={() => skipPrev(sentences, rate, shadowMode, shadowPause)}
            disabled={!isActive || currentIndex <= 0}
            className="flex items-center gap-1 px-3 py-3 rounded-xl bg-gray-100 hover:bg-gray-200 disabled:opacity-40 transition-all"
            title="이전 문장"
          >
          ...
          <button
            onClick={() => replayCurrent(sentences, rate, shadowMode, shadowPause)}
            disabled={!isActive}
            className="flex items-center gap-1 px-3 py-3 rounded-xl bg-gray-100 hover:bg-gray-200 disabled:opacity-40 transition-all"
            title="현재 문장 다시"
          >
          ...
          <button
            onClick={() => skipNext(sentences, rate, shadowMode, shadowPause)}
          ...
          return (
            <div
              ...
              className={`group flex gap-3 p-3 rounded-xl cursor-pointer transition-all ${
                isCurrentSentence
                  ? 'bg-indigo-50 border-2 border-indigo-400 sentence-active'
